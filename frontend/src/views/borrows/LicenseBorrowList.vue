  ...
  <div class="license-borrow-list">
    ...
    <div class="table-container">
      ...
      <el-table :data="tableData" border stripe>
        <el-table-column prop="license_name" label="证照名称" />
    ...
    <el-dialog v-model="borrowDialogVisible" title="证照借出登记" width="500px">
      <el-form :model="borrowForm" :rules="borrowRules" ref="borrowFormRef" label-width="100px">
        <el-form-item label="证照" prop="license_id">
          <el-select v-model="borrowForm.license_id" placeholder="请选择证照" style="width: 100%">
            <el-option v-for="license in licenseList" :key="license.id" :label="license.name" :value="license.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="备注">
          <el-input v-model="borrowForm.remark" type="textarea" :rows="2" placeholder="请输入备注" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const borrowRules = {
  license_id: [{ required: true, message: '请选择证照', trigger: 'change' }],
...
const loadData = async () => {
  try {
    ...
    tableData.value = await api.get('/borrows/licenses', { params })
  } catch (e) {
    console.error(e)
  }
}
...
const loadLicenses = async () => {
  try {
    licenseList.value = await api.get('/licenses?status=valid')
  } catch (e) {
    console.error(e)
  }
}
...
const openBorrowDialog = () => {
  Object.assign(borrowForm, { license_id: '', borrow_time: '', remark: '' })
...
const handleBorrow = async () => {
  ...
  try {
    await api.post('/borrows/licenses', {
      ...borrowForm,
...
const handleReturn = async () => {
  try {
    await api.put(`/borrows/licenses/${currentBorrow.value.id}/return`, returnForm)
